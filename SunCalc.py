...
class sun:
    ...
    def __timefromdecimalday(day,when):
        ...
        s = int(seconds)%60
        return  when.replace(hour=h,minute=m,second=s,microsecond=0)
    ...
    def __calc(self):
        ...
        Seqcent = sin(rad(Manom)) * (1.914602 - Jcent *
                                     (0.004817 + 0.000014 * Jcent)) + sin(rad(2 * Manom)) * (0.019993 -
        ...
        eqtime = 4 * deg(vary * sin(2 * rad(Mlong)) - 2 * Eccent *
                         ...
                         cos(2 * rad(Mlong)) - 0.5 * vary * vary *
                         sin(4 * rad(Mlong)) - 1.25 * Eccent * Eccent *
                         sin(2 * rad(Manom)))
...
if __name__ == "__main__":
    s = sun(lat=45.54, long=-73.61)
    t=r=datetime.now(timezone.utc).astimezone().replace(microsecond=0)
    sunrise=s.sunrise(t)
    sunset=s.sunset(t)

    print ("%s %s"%(s.lat,s.long))
    print ("Today : %40s\nSunrise:%40s\nSunset :%40s"%(t,sunrise,sunset))
    ...
    if ((sunrise<t) and (t<sunset)):
        print ("day since %15s"%datetime.utcfromtimestamp(t-sunrise).time())
        print ("night in  %15s"%datetime.utcfromtimestamp(sunset-t).time())
    else:
        if (t<sunrise):
            #Same Day
            print ("night since %15s"%datetime.utcfromtimestamp((t-s.sunset(r-timedelta(days=1)).timestamp())).time())
